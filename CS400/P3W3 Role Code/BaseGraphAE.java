...
public class BaseGraphAE <NodeType, EdgeType extends Number> {
    ...
    protected class Node {
        ...
        public Node(CityAE city) { this.city = city;}
    }
    // Nodes can be retrieved from this hashtable by their unique data
    protected Hashtable<CityAE,Node> nodes = new Hashtable();

    // Each edge contains data/weight, and two nodes that it connects
    protected class Edge {
        public double data; // the weight or cost of this edge
        public Node predecessor;
        public Node successor;
        public Edge(double data, Node pred, Node succ) {
            this.data = data;
     ...
     */
    public boolean insertNode(CityAE city) {
        if(nodes.containsKey(city)) return false; // throws NPE when data's null
     ...
     * @throws NullPointerException if data is null
     */
    public boolean removeNode(CityAE city) {
        // remove this node from nodes collection
        if(!nodes.containsKey(city)) return false; // throws NPE when data==null
        Node oldNode = nodes.remove(city);
     ...
     * @return true if data item is stored in a node within the graph, or
               false otherwise
     */
    public boolean containsNode(CityAE city) {
     ...
     */
    public boolean insertEdge(NodeType pred, NodeType succ, double weight) {
     ...
     */
    public double getEdge(NodeType pred, NodeType succ) {
    ...
    protected Edge getEdgeHelper(NodeType pred, NodeType succ) {
        ...
        for(Edge edge : predNode.edgesLeaving)
            ...
            if(edge.successor.city.equals(succ))
                return edge;
    ...
    public CityAE getNode(String nodeName) {
        CityAE returnNode = null;
        Set<CityAE> keys = nodes.keySet();
        for (CityAE key : keys) {
            if (key.getCityName().equals(nodeName)) {
                returnNode = key;
            }
        }
        return returnNode;
